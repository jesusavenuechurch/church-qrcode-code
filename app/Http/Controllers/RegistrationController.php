...
class RegistrationController extends Controller
{
    ...
    public function register(Request $request, $orgSlug, $eventSlug)
    {
        ...
        try {
            ...
            for ($i = 1; $i <= $quantityPerPurchase; $i++) {
                ...
                $ticketNumber = $this->generateTicketNumber($event->id);
                $qrCode = 'QR-' . Str::uuid();
    ...
    {
        $lastTicket = Ticket::where('event_id', $eventId)
            ->orderBy('id', 'desc')
            ->first();

        $number = $lastTicket ? intval(substr($lastTicket->ticket_number, -5)) + 1 : 1;

        return 'TKT-' . $eventId . '-' . str_pad($number, 5, '0', STR_PAD_LEFT);
    }
}